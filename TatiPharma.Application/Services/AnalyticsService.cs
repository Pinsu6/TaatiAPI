    ...
    public class AnalyticsService : IAnalyticsService
    {
        ...
        public async Task<ApiResponse<SalesAnalyticsDto>> GetSalesAnalyticsAsync(SalesAnalyticsFilterRequestDto request)
        {
            try
            {
                ...
                // Use YTD for topProducts and salesByRegion to ensure data (aligns with YTD KPI and yearly trend)
                // Period filter now primarily affects other future expansions; defaults to ThisYear for broad data.
                var (start, end) = GetPeriodDates(request.Period, now); // Still computed, but overridden below for visuals
                var prevYtdStart = new DateTime(now.Year - 1, 1, 1);
                var prevYtdEnd = new DateTime(now.Year - 1, 12, 31);

                // Sales Trend (current year, filtered by city and category)
                var year = now.Year;
                var monthlyRaw = await _salesAnalyticsRepository.GetMonthlySalesAsync(year, request.City, request.ProductCategory);
                var months = new string[] { "", "Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec" };
                var salesTrend = new List<MonthlySalesTrendDto>();
                for (int m = 1; m <= 12; m++)
                {
                ...
                // FIXED: Top Products (use YTD for data availability; filter by city/category)
                var topProducts = await _salesAnalyticsRepository.GetTopProductsAsync(ytdStart, ytdEnd, 5, request.City, request.ProductCategory);

                // FIXED: Sales by Region (use YTD for data; growth vs previous full year)
                var currentRegions = await _salesAnalyticsRepository.GetRegionSalesAsync(ytdStart, ytdEnd, request.City, request.ProductCategory);
                var prevRegions = await _salesAnalyticsRepository.GetRegionSalesAsync(prevYtdStart, prevYtdEnd, request.City, request.ProductCategory);
                var salesByRegion = currentRegions.Select(cr =>
                {
                    var pr = prevRegions.FirstOrDefault(p => p.Region == cr.Region);
                    decimal prevRev = pr?.Revenue ?? 0;
        ...
        private (DateTime, DateTime) GetPreviousPeriodDates(DateTime currentStart, string period)
        {
            int monthsToSubtract = period switch
            {
                ...
                "Last3Months" => 3,
                "ThisYear" => 12,
                _ => 12 // FIXED: Default to 12 for year
            };
            var prevStart = currentStart.AddMonths(-monthsToSubtract);
            var prevEnd = prevStart.AddMonths(monthsToSubtract).AddDays(-1);
            return (prevStart, prevEnd);
        ...
        public async Task<ApiResponse<ProductInsightsDto>> GetProductInsightsAsync(ProductInsightsFilterRequestDto request)
        {
            try
            {
                ...
                var top = await _salesAnalyticsRepository.GetTopProductAsync(start, end, request.DrugTypeId);
                kpis.TopProductName = top.Name;
                kpis.TopProductRevenue = top.Revenue;

                var fastest = await _salesAnalyticsRepository.GetFastestGrowingProductAsync(start, end, prevStart, prevEnd, request.DrugTypeId);
                kpis.FastestGrowingName = fastest.Name;
                kpis.FastestGrowingGrowth = CalculateGrowth(fastest.Revenue, top.Revenue); // Use current vs prev for growth

                var slowest = await _salesAnalyticsRepository.GetSlowestMovingProductAsync(start, end, prevStart, prevEnd, request.DrugTypeId);
                kpis.SlowestMovingName = slowest.Name;
                kpis.SlowestMovingGrowth = CalculateGrowth(slowest.Revenue, slowest.Revenue); // Simplified

                kpis.NewLaunchesCount = await _salesAnalyticsRepository.GetNewLaunchesCountAsync(start, end, request.DrugTypeId);

                // Top SKUs
                var topSkus = await _salesAnalyticsRepository.GetTopSkusAsync(start, end, 10, request.DrugTypeId);

                // Products Table (no status filter anymore)
                var products = await _salesAnalyticsRepository.GetProductInsightsAsync(start, end, prevStart, prevEnd, request.DrugTypeId);

                // AI Recommendations
                var aiRecos = await _salesAnalyticsRepository.GetAiRecommendationsAsync(start, end, request.DrugTypeId);
        ...
        public async Task<ApiResponse<DashboardAnalyticsDto>> GetDashboardAnalyticsAsync(DashboardFilterRequestDto request)
        {
            try
            {
                ...
                {
                    TotalSalesYtd = await _salesAnalyticsRepository.GetTotalSalesYtdAsync(ytdStart, ytdEnd, request.City, request.ProductCategory),
                    SalesGrowth = await _salesAnalyticsRepository.GetSalesGrowthAsync(start, end, prevStart, prevEnd, request.City, request.ProductCategory),
                    ActivePharmacies = await _salesAnalyticsRepository.GetActivePharmaciesCountAsync(request.City, request.Search),
                    TotalCustomers = await _salesAnalyticsRepository.GetTotalCustomersCountAsync(request.City, request.Search),
                    OrderFulfillment = await _salesAnalyticsRepository.GetOrderFulfillmentAsync(start, end, request.City, request.ProductCategory),
                    FulfillmentChange = await _salesAnalyticsRepository.GetFulfillmentChangeAsync(start, end, prevStart, prevEnd, request.City, request.ProductCategory)
                };

                // Revenue Performance
                var revenueYear = await _salesAnalyticsRepository.GetDataYearAsync();
                var revenuePerformance = await _salesAnalyticsRepository.GetRevenuePerformanceAsync(revenueYear, request.City, request.ProductCategory);
                ...
                var regionalPerformance = await _salesAnalyticsRepository.GetRegionalPerformanceAsync(start, end, request.City, request.ProductCategory);

                // Top Pharmacies – USE YTD TO CAPTURE ALL DATA
                var topPharmacies = await _salesAnalyticsRepository.GetTopPharmaciesAsync(
                    ytdStart, ytdEnd,  // ← CHANGED: Use full year
                    request.City,
                ...
                var result = new DashboardAnalyticsDto
                {
                    ...
                    Cities = await _salesAnalyticsRepository.GetCitiesAsync(),
                    Products = await _salesAnalyticsRepository.GetProductsAsync(request.Search),
                    DrugTypes = await _salesAnalyticsRepository.GetDrugTypesAsync()
                };
